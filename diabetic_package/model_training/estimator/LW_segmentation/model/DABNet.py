...
class DABModule(nn.Module):
    def __init__(self, nIn, d=1, kSize=3, dkSize=3):
        ...
        self.conv1x1 = Conv_norelu(nIn // 2, nIn, 1, 1, padding=0, bn_acti=False)
...
class DownSamplingBlock(nn.Module):
    def __init__(self, nIn, nOut):
        ...
        self.conv3x3 = Conv_norelu(nIn, nConv, kSize=3, stride=2, padding=1)

        self.max_pool = nn.MaxPool2d(2, stride=2,padding=(0,0))
        self.max_pool_p1 = nn.MaxPool2d(2, stride=2,padding=(1,0))
        self.max_pool_p2 = nn.MaxPool2d(2, stride=2,padding=(0,1))
        self.max_pool_p3 = nn.MaxPool2d(2, stride=2,padding=(1,1))
        self.bn_prelu = BNPReLU(nOut)
    ...
    def forward(self, input):
        ...
        if self.nIn < self.nOut:
            ...
            # # if output.size()[2]<max_pool.size()[2]:
            # if output.size()[2] != max_pool.size()[2] and output.size()[3] != \
            #         max_pool.size()[3]:
            #     max_pool = self.max_pool_p3(input)
            #     # elif output.size()[3]<max_pool.size()[3]:
            # elif output.size()[2] != max_pool.size()[2]:
            #     max_pool = self.max_pool_p1(input)
            #
            # elif output.size()[3] != max_pool.size()[3]:
            #     max_pool = self.max_pool_p2(input)
...
class DABNet(nn.Module):
    def __init__(self, classes=2, block_1=3, block_2=6):
        ...
        # self.downsample_1 = torch.jit.script(DownSamplingBlock(32 + 3, 64))
        ...
        # self.downsample_2 = torch.jit.script(DownSamplingBlock(128 + 3, 128))
        ...
        self.classifier = nn.Sequential(Conv_norelu(259, classes, 1, 1, padding=0))
