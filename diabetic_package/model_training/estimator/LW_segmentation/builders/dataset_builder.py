...
from diabetic_package.model_training.estimator.LW_segmentation.datasets import DataSet, ValDataSet, TrainInform, TestDataSet, create_data




def build_dataset_train(data_dir, classes_num, input_size, batch_size,
                        ignore_label,train_type, random_scale, random_mirror,
                        num_workers):
    ...
    trainLoader = data.DataLoader(
        DataSet(train_dir, crop_size=input_size, scale=random_scale,
                      mirror=random_mirror, mean=datas['mean'],ignore_label=ignore_label),
...
def build_dataset_test(data_dir, class_num, num_workers,batch_size, none_gt=False ,mode="train"):
    inform_data_file =  './dataset_inform.pkl'
    # data_list = np.sort(bz_path.get_file_path(data_dir + "/img/", ret_full_path=True))
