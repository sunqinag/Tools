...
class ModelMain(nn.Module):
    ...
    def _make_cbl(self, _in, _out, ks,pad=0):
    ...
    def _make_embedding(self, filters_list, in_filters, out_filter):
        m = nn.ModuleList([
            ...
            self._make_cbl(filters_list[0], filters_list[1], 3,1),
    ...
    def forward(self, x):
        def _branch(_embedding, _in):
            # print(len(_embedding))
            _in = _embedding[0](_in)
            _in = _embedding[1](_in)
            _in = _embedding[2](_in)
            _in = _embedding[3](_in)
            _in = _embedding[4](_in)
            out_branch = _in
            _in = _embedding[5](_in)
            _in = _embedding[-1](_in)
        ...
        # print('out0.shape:',out0.shape,'out0_branch.shape:',out0_branch.shape)
        #  yolo branch 1
        x1_in = self.embedding1_cbl(out0_branch)
        x1_in = self.embedding1_upsample(x1_in)
        x1_in = torch.cat([x1_in, x1], 1)
        out1, out1_branch = _branch(self.embedding1, x1_in)
        # print('out1.shape:', out1.shape, 'out1_branch.shape:', out1_branch.shape)
        #  yolo branch 2
    ...
    def get_loss(self, labels):
        ...
        for _ in range(len(losses_name)):
            losses.append([])
        for i in range(3):
            _loss_item = self.yolo_losses[i](self.outputs[i], labels)
