...
def create_data(data_dir):
    folders = np.sort(bz_path.get_subfolder_path(data_dir,
                                                 ret_full_path=True,
                                                 is_recursion=False))
    feature_set = []
    labels_set = []
    for i, file in enumerate(folders):
        print(file)
        img_paths = np.sort(bz_path.get_file_path(file + "/img",
                                                  ret_full_path=True))
        label_paths =  np.sort(bz_path.get_file_path(file + "/label",
                                                     ret_full_path=True))
...
class DataSet(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = np.asarray(img_pad[h_off: h_off + self.crop_h,
                           w_off: w_off + self.crop_w], np.float32)
        label = np.asarray(label_pad[h_off: h_off + self.crop_h,
                           w_off: w_off + self.crop_w], np.float32)
...
class ValDataSet(data.Dataset):
    ...
    def __init__(self,
        ...
        for img, label in zip(img_list, label_list):
            name = img + " " + label
            self.files.append({"img": img, "label": label, "name": name})
    ...
    def readWholeTrainSet(self, fileName, train_flag=True):
        ...
        for img_file, label_file in  zip(self.train_set_file, self.label_set_file):
            ...
            if max_val > (self.classes - 1) or min_val < 0:
                ...
                print('Some problem with labels. Please check. label_set:',
                      unique_values)
